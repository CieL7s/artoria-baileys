// [RUST-DELEGATED] This file is a thin N-API bridge. Original JS logic archived at ./encode.legacy.js
...
export const encodeBinaryNode = (node, opts = constants, buffer = [0]) => {
    // [P2] Object Binding — measured regresi untuk semua ukuran (encode small 0.42x vs JSON 1.11x, large 0.65x vs JSON 1.33x).
    // Default OFF, JSON string tetap default. Aktifkan via WABINARY_OBJECT=1 untuk eksperimen.
    if (process.env.WABINARY_OBJECT === '1' && typeof rust.encodeBinaryNodeObject === 'function') {
        try {
            return rust.encodeBinaryNodeObject(node);
        } catch {}
    }
    try {
        return rust.encodeBinaryNode(JSON.stringify(node));
    } catch (err) {
