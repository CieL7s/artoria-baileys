// [RUST-DELEGATED] This file is a thin N-API bridge. Original JS logic archived at ./decode.legacy.js
...
const mapContentBuffers = (node) => {
    if (!node) return node;
    if (Array.isArray(node.content)) {
        node.content = node.content.map(mapContentBuffers);
    } else if (node.content && node.content.type === 'Buffer' && Array.isArray(node.content.data)) {
        node.content = Buffer.from(node.content.data);
    }
    return node;
};
...
export const decodeDecompressedBinaryNode = (buffer, opts = constants, indexRef = { index: 0 }) => {
    // [P2] Object Binding (direct N-API object) — measured 0.01-0.42x (lebih lambat dari JSON untuk semua ukuran, terutama large 0.01x vs JSON 0.028x).
    // Default OFF, JSON string tetap lebih cepat untuk semua ukuran. Aktifkan via WABINARY_OBJECT=1 untuk eksperimen.
    if (process.env.WABINARY_OBJECT === '1' && typeof rust.decodeBinaryNodeObject === 'function') {
        try {
            const obj = rust.decodeBinaryNodeObject(buffer);
            return mapContentBuffers(obj);
        } catch {}
    }
    try {
        const jsonStr = rust.decodeBinaryNode(buffer);
        const parsed = JSON.parse(jsonStr);
        return mapContentBuffers(parsed);
    } catch (err) {
        return decodeDecompressedBinaryNodeInner(buffer, opts, indexRef);
    }
};

const decodeDecompressedBinaryNodeInner = (buffer, opts, indexRef = { index: 0 }) => {
