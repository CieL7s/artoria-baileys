...
const useRust = process.env.SIGNAL_ENGINE !== 'js' && rust && typeof rust.decodeMessageNode === 'function';
...
export const extractAddressingContext = (stanza) => {
    if (useRust && stanza) {
        try {
            const res = rust.extractAddressingContext(JSON.stringify(stanza));
...
export function decodeMessageNode(stanza, meId, meLid) {
    if (useRust && stanza) {
        try {
            const res = rust.decodeMessageNode(JSON.stringify(stanza), meId, meLid);
            if (res) return JSON.parse(res);
        } catch (e) {
            const msg = e.message || '';
            if (msg.includes('missing id attribute')) {
                throw new Boom('Invalid message stanza: missing id attribute', { data: stanza });
            }
            if (msg.includes('missing from attribute')) {
                throw new Boom('Invalid message stanza: missing from attribute', { data: stanza });
            }
            if (msg.includes('receipient present, but msg not from me')) {
                throw new Boom('receipient present, but msg not from me', { data: stanza });
            }
            if (msg.includes('No participant in group message')) {
    ...
    if (isPnUser(from) || isLidUser(from) || isHostedLidUser(from) || isHostedPnUser(from)) {
        if (recipient && !isJidMetaAI(recipient)) {
            if (!isMe(from) && !isMeLid(from)) {
                if (areJidsSameUser(from, recipient)) {
                    fromMe = true;
                } else {
                    throw new Boom('receipient present, but msg not from me', { data: stanza });
                }
        ...
        async decrypt() {
            ...
            if (Array.isArray(stanza.content)) {
                for (const { tag, attrs, content } of stanza.content) {
                    let rawBytes = content;
                    if (rawBytes && typeof rawBytes === 'object' && rawBytes.data && Array.isArray(rawBytes.data)) {
                        rawBytes = Buffer.from(rawBytes.data);
                    } else if (Array.isArray(rawBytes)) {
                        rawBytes = Buffer.from(rawBytes);
                    }
                    if (tag === 'verified_name' && (rawBytes instanceof Uint8Array || Buffer.isBuffer(rawBytes))) {
                        const cert = proto.VerifiedNameCertificate.decode(rawBytes);
                    ...
                    if (!(rawBytes instanceof Uint8Array || Buffer.isBuffer(rawBytes))) {
                        continue;
                    }
                    decryptables += 1;
                    let msgBuffer;
                    ...
                    try {
                        ...
                        console.log('[E2EE Decrypt Attempt]:', e2eType, 'jid:', decryptionJid, 'author:', author, 'bytes:', rawBytes.length);
                        switch (e2eType) {
                            case 'skmsg':
                        ...
                        if (!msgBuffer) {
                            throw new Error(`Failed to decrypt ${e2eType}: empty buffer returned`);
                        }
                        const cleanBuf = e2eType !== 'plaintext' ? unpadRandomMax16(Buffer.from(msgBuffer)) : Buffer.from(msgBuffer);
                        let msg = proto.Message.decode(Buffer.from(cleanBuf));
                        ...
                        console.log('[E2EE Decrypt Failure]:', err?.message, err?.stack);
