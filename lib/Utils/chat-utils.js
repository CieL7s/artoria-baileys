...
import { aesDecrypt, aesEncrypt, hmacSign, hkdf } from './crypto.js';
...
export const expandAppStateKeys = (keyData) => {
    const key = hkdf(keyData, 160, { info: 'Sync Action' });
    return {
        indexKey: key.subarray(0, 32),
        valueEncryptionKey: key.subarray(32, 64),
        valueMacKey: key.subarray(64, 96),
        snapshotMacKey: key.subarray(96, 128),
        patchMacKey: key.subarray(128, 160)
    };
};
