...
export function expandAppStateKeys(keydata) {
    const key = Buffer.isBuffer(keydata) ? keydata : Buffer.from(keydata);
    const keyExpanded = Buffer.from(hkdf(key, 160, { info: 'Mutation Keys' }));
    return {
        indexKey: keyExpanded.subarray(0, 32),
        valueEncryptionKey: keyExpanded.subarray(32, 64),
        valueMacKey: keyExpanded.subarray(64, 96),
        snapshotMacKey: keyExpanded.subarray(96, 128),
        patchMacKey: keyExpanded.subarray(128, 160)
    };
}
