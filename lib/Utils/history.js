...
const useRust = process.env.SIGNAL_ENGINE !== 'js' && rust && typeof rust.historyProcessMessage === 'function';

const inflatePromise = promisify(inflate);

export const extractPnFromMessages = (messages) => {
    if (useRust && Array.isArray(messages)) {
        try {
            const res = rust.historyExtractPnFromMessages(JSON.stringify(messages));
...
export const processHistoryMessage = (item, logger) => {
    if (useRust && item) {
        try {
            const res = rust.historyProcessMessage(JSON.stringify(item));
