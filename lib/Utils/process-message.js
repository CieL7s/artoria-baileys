...
export const cleanMessage = (message, meId, meLid) => {
    if (useRust && message) {
        try {
            const res = rust.cleanMessage(JSON.stringify(message), meId, meLid || null);
            if (res) {
                const cleaned = JSON.parse(res);
                if (cleaned.key) {
                    Object.assign(message.key, cleaned.key);
                }
                if (cleaned.message) {
                    message.message = cleaned.message;
                }
                return;
            }
        } catch (e) {}
...
export const isRealMessage = (message) => {
    if (useRust && message) {
        try {
            return rust.isRealMessage(JSON.stringify(message));
        } catch (e) {}
    }
...
export const shouldIncrementChatUnread = (message) => {
    if (useRust && message) {
        try {
            return rust.shouldIncrementChatUnread(JSON.stringify(message));
        } catch (e) {}
    }
...
export const getChatId = ({ remoteJid, participant, fromMe }) => {
    ...
    if (useRust) {
        try {
            return rust.getChatId(remoteJid, participant || null, !!fromMe);
        } catch (e) {
            throw new Boom(e.message || 'Cannot derive chat id', {
                data: { remoteJid, participant, fromMe }
            });
        }
    }
