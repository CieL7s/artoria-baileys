...
const useRust = process.env.SIGNAL_ENGINE !== 'js' && rust && typeof rust.cleanMessage === 'function';
...
export const cleanMessage = (message, meId, meLid) => {
    ...
    function normaliseKey(msgKey) {
        if (!msgKey) return;
        if (isHostedPnUser(msgKey.remoteJid) || isHostedLidUser(msgKey.remoteJid)) {
            msgKey.remoteJid = jidEncode(jidDecode(msgKey.remoteJid)?.user, isHostedPnUser(msgKey.remoteJid) ? 's.whatsapp.net' : 'lid');
        } else if (msgKey.remoteJid) {
            msgKey.remoteJid = jidNormalizedUser(msgKey.remoteJid);
        }
        if (isHostedPnUser(msgKey.participant) || isHostedLidUser(msgKey.participant)) {
            msgKey.participant = jidEncode(jidDecode(msgKey.participant)?.user, isHostedPnUser(msgKey.participant) ? 's.whatsapp.net' : 'lid');
        } else if (msgKey.participant) {
            msgKey.participant = jidNormalizedUser(msgKey.participant);
        }
...
export function decryptPollVote({ encPayload, encIv }, { pollCreatorJid, pollMsgId, pollEncKey, voterJid }) {
    if (useRust) {
        try {
            const buf = rust.decryptPollVote(
                Buffer.from(encPayload),
                Buffer.from(encIv),
                pollCreatorJid,
                pollMsgId,
                Buffer.from(pollEncKey),
                voterJid
            );
            if (buf) {
                return proto.Message.PollVoteMessage.decode(buf);
            }
        } catch (e) {
            throw new Error(`Failed to decrypt poll vote: ${e.message || e}`);
        }
    }
...
export function decryptEventResponse({ encPayload, encIv }, { eventCreatorJid, eventMsgId, eventEncKey, responderJid }) {
    if (useRust) {
        try {
            const buf = rust.decryptEventResponse(
                Buffer.from(encPayload),
                Buffer.from(encIv),
                eventCreatorJid,
                eventMsgId,
                Buffer.from(eventEncKey),
                responderJid
            );
            if (buf) {
                return proto.Message.EventResponseMessage.decode(buf);
            }
        } catch (e) {
            throw new Error(`Failed to decrypt event response: ${e.message || e}`);
        }
    }
    const sign = Buffer.concat([
        toBinary(eventMsgId),
