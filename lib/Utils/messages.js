...
const useRust = process.env.SIGNAL_ENGINE !== 'js' && rust && typeof rust.normalizeMessageContent === 'function';
const MIMETYPE_MAP = {
    image: 'image/jpeg',
    video: 'video/mp4',
...
export const getContentType = (content) => {
    if (content) {
        if (useRust) {
            try {
                const res = rust.getContentType(JSON.stringify(content));
                if (res) return res;
            } catch (e) {}
        }
...
export const normalizeMessageContent = (content) => {
    ...
    if (useRust) {
        try {
            ...
            const resJson = rust.normalizeMessageContent(rawJson);
            if (resJson) {
...
export const extractMessageContent = (content) => {
    ...
    if (useRust) {
        try {
            const rawJson = JSON.stringify(content);
            const resJson = rust.extractMessageContent(rawJson);
            if (resJson) {
                return JSON.parse(resJson);
            }
        } catch (e) {}
...
export const getDevice = (id) => {
    if (useRust && typeof id === 'string') {
        try {
            const dev = rust.getDevice(id);
            if (dev && dev !== 'unknown') return dev;
        } catch (e) {}
    }
