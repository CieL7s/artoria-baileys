...
export const hkdf = (buffer, expandedLength, info = {}) => {
    const salt = info.salt ? Buffer.from(info.salt) : Buffer.alloc(32);
    const infoBuf = info.info ? Buffer.from(info.info) : Buffer.alloc(0);
    const prk = createHmac('sha256', salt).update(buffer).digest();
    let prev = Buffer.alloc(0);
    const outputBuffers = [];
    const numBlocks = Math.ceil(expandedLength / 32);
    for (let i = 0; i < numBlocks; i++) {
        ...
        prev = hmac.digest();
        outputBuffers.push(prev);
    }
    return Buffer.concat(outputBuffers).subarray(0, expandedLength);
};
