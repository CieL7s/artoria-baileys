...
export function hkdf(key, length, { salt = Buffer.alloc(32, 0), info = '' } = {}) {
    const saltBuf = Buffer.isBuffer(salt) ? salt : (salt?.length ? Buffer.from(salt) : Buffer.alloc(32, 0));
    const infoBuf = Buffer.isBuffer(info) ? info : Buffer.from(info || '', 'utf-8');
    return Buffer.from(hkdfSync('sha256', Buffer.from(key), saltBuf, infoBuf, length));
}
