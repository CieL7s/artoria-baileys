...
export class LIDMappingStore {
    ...
    async storeLIDPNMappings(pairs) {
        ...
        let validatedPairs;
        try {
            const json = rust.signalLidValidatePairs(JSON.stringify(pairs));
            const parsed = JSON.parse(json);
            validatedPairs = parsed.map(p => ({ pnUser: p.pn_user, lidUser: p.lid_user }));
        } catch (err) {
            this.logger.warn({ err: err?.message || err }, '[LID_MAPPING_RUST_FALLBACK] Native Rust pair validation failed, using JS fallback');
            validatedPairs = [];
            for (const { lid, pn } of pairs) {
                if (!((isLidUser(lid) && isPnUser(pn)) || (isPnUser(lid) && isLidUser(pn)))) continue;
                const lidDecoded = jidDecode(lid);
                const pnDecoded = jidDecode(pn);
                if (lidDecoded && pnDecoded) validatedPairs.push({ pnUser: pnDecoded.user, lidUser: lidDecoded.user });
            }
        }
    ...
    async _getLIDsForPNsImpl(pns) {
        ...
        const addResolvedPair = (pn, decoded, lidUser) => {
            ...
            const deviceSpecificLid = rust.signalLidResolvePnToLid(pn, normalizedLidUser) || `${normalizedLidUser}${decoded.device ? `:${decoded.device}` : ``}@${decoded.server === 'hosted' ? 'hosted.lid' : 'lid'}`;
            this.logger.trace(`getLIDForPN: ${pn} → ${deviceSpecificLid} (user mapping with device ${decoded.device || 0})`);
    ...
    async _getPNsForLIDsImpl(lids) {
        ...
        const addResolvedPair = (lid, decoded, pnUser) => {
            ...
            const pnJid = rust.signalLidResolveLidToPn(lid, pnUser) || `${pnUser}${decoded.device ? `:${decoded.device}` : ``}@${decoded.domainType === WAJIDDomains.HOSTED_LID ? 'hosted' : 's.whatsapp.net'}`;
