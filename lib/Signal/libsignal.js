...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
    ...
    const repository = {
        ...
        async processSenderKeyDistributionMessage({ item, authorJid }) {
            ...
            return parsedKeys.transaction(async () => {
                ...
                try {
                    ...
                    const { recordSkdm } = await import('../../test/tools/traffic-recorder.js');
                    recordSkdm({
        ...
        async decryptMessage({ jid, type, ciphertext }) {
            ...
            if (type === 'pkmsg') {
                const identityKey = extractIdentityFromPkmsg(buf);
                if (identityKey) {
            ...
            async function doDecrypt() {
                ...
                try {
                    const { recordPairwise } = await import('../../test/tools/traffic-recorder.js');
                    recordPairwise({
                        jid,
                        type,
                        inputBytes: buf,
        ...
        async encryptMessage({ jid, data }) {
            const addr = jidToSignalProtocolAddress(jid);
            console.log('[SIGNAL_ENCRYPT_MESSAGE_START]', { jid, addr: addr.toString() });
