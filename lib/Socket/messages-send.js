...
export const makeMessagesSocket = (config) => {
    ...
    const assertSessions = async (jids, force) => {
        ...
        if (jidsRequiringFetch.length) {
            // LID if mapped, otherwise original PN
            const pnJids = jidsRequiringFetch.filter(jid => !!isPnUser(jid) || !!isHostedPnUser(jid));
            const lidMappings = (await signalRepository.lidMapping.getLIDsForPNs(pnJids)) || [];
            const mappedLids = lidMappings.map(a => a.lid);
            const mappedPns = new Set(lidMappings.map(a => a.pn));
            const unmappedPns = pnJids.filter(pn => !mappedPns.has(pn));
            ...
            console.log('[ASSERT_SESSIONS_IQ_RESULT]', { wireJids, result: JSON.stringify(result) });
    ...
    const createParticipantNodes = async (recipientJids, message, extraAttrs, dsmMessage) => {
        ...
        const encryptionPromises = patchedMessages.map(async ({ recipientJid: jid, message: patchedMessage }) => {
            try {
                ...
                console.error('[ENCRYPT_PER_DEVICE_ERROR]', { jid, errorMessage: err?.message, errorStack: err?.stack, err });
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                console.log('[RELAY_ASSERT_SESSIONS_DONE]', { allRecipients, meRecipients, otherRecipients });
